...
static long read_line(FILE* file, char** buffer, size_t* capacity){
    ...
    if(*buffer == NULL || *capacity == 0){
        ...
        *buffer = malloc(*capacity);
        if(!*buffer){
            MALLOC_ERROR("failed line buffer allocation");
            return -1;
        }
    }
    ...
    while((c = fgetc(file)) != EOF){
        if(len + 1 >= *capacity){
            ...
            if(!next){
                MALLOC_ERROR("failed line buffer resize");
                return -1;
            }
            *buffer = next;
...
CellProtected* read_protected(const char* filename){
    ...
    while(read_line(file, &buffer, &buff_size) > 1){
...
void create_dir_structure(){
    if(MKDIR(BLOCK_STORAGE_DIR) == -1 && errno != EEXIST){
        FILE_ERROR("cannot create file structure!");
        fprintf(stderr, "%s", strerror(errno)); 
    }
    if(MKDIR(BLOCKCHAIN_DIR) == -1 && errno != EEXIST){
        FILE_ERROR("cannot create file structure!");
        fprintf(stderr, "%s", strerror(errno)); 
    }
    if(MKDIR(BLOCK_STORAGE_DIR "temp/") == -1 && errno != EEXIST){
        FILE_ERROR("cannot create file structure!");
        fprintf(stderr, "%s", strerror(errno)); 
    }
}
