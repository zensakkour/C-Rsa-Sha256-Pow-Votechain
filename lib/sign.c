...
static char* copy_substr(const char* src, size_t len){
    char* ret = malloc(len + 1);
    if(!ret){
        MALLOC_ERROR("substring copy allocation failed");
        return NULL;
    }
    memcpy(ret, src, len);
    ret[len] = '\0';
...
char* signature_to_str(const Signature* sgn){
    ...
    int i;
    for(i=0; i < sgn->len; i++) {
        sprintf(buffer, "%llx" , (unsigned long long)sgn->xs[i]);
        const size_t bufflen = strlen(buffer);
        assert(bufflen <= 9);
        size_t j;
        for(j=0; j<bufflen; j++) {
            assert((size_t)pos < maxsize);
            result[pos] = buffer[j];
            pos = pos +1;
        }
        result[pos] = '#';
    ...
    for(i=0; i<len; i++) {
        if(str[i] != '#'){
            ...
            if(pos != 0) {
                ...
                if(1 != sscanf(buffer , "%llx" , &parsed)){
                    fprintf(stderr, "sscanf error\n");
                    free(content);
                    return NULL;
                }
                content[num] = (int64)parsed;
                num = num + 1;
                pos = 0;
            }
        }
...
Protected* str_to_protected_len(const char* str, size_t slen){
    ...
    char* msg = copy_substr(str+msg_s, msg_e-msg_s+1);
    if(!msg){
